...
В mock неизвестные сведения определяются по введённым полям, а вопросы — шаблонные с контекстом описания. В режиме OpenAI известные поля извлекаются из текста одновременно с генерацией вопросов; дополнительного платного запроса для этого нет.
...
Сервер восстанавливает исходную цитату при отличиях регистра, пробелов и завершающей пунктуации. Поле, для которого AI предложил отсутствующий в исходных данных текст, очищается; в `ai.warning` указываются имена таких полей. Остальная карточка возвращается без потерь. Пропущенный моделью контекст заполняется исходным описанием (до 5000 символов), если пользователь не задал его явно. Это не подтверждает достоверность или полноту карточки — человеку нужно проверить смысл каждого поля.
